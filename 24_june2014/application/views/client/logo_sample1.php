...
?>
           ...
           foreach($samples as $row) {
            ...
            Duis tincidunt lectus quis dui viverra vestibulum. Suspendisse vulputate aliquam dui.<br/>
            Nulla elementum dui ut augue. Aliquam vehicula mi at mauris.<br/><br/>
            Maecenas placerat, nisl at consequat rhoncus, sem nunc gravida justo, quis eleifend arcu velit quis lacus.
            Morbi magna magna, tincidunt a, mattis non, imperdiet vitae, tellus. Sed odio est, auctor ac, sollicitudin in, consequat vitae, orci.
		   ...
		   $designer_post_id = $row['designer_post_order_id'];
			...
			{	
				$oldORnew = 'new';			
				$data['logos'] = $this->clientmodel->getAll( 'tbl_logo_samples', 'designer_post_id', array('designer_post_id'=>$designer_post_id,'sample_type'=>'logo'));
		    ...
		    if($oldORnew == 'new') {
				...
				if(!empty($logos)){
					echo '<tr><td>';
					foreach($data['logos'] as $row1){
						if (@fopen(PATH_SAMPLE.'thumb_'.$row->sample_name, "r") && $row1->sample_name!='') {
