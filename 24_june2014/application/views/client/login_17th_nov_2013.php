...
<link href="<?php echo base_url()?>css_client/reset.css" rel="stylesheet" type="text/css" />
<!--[if lt IE 8]>
...
.reset
{
    background:url(<?= base_url(); ?>images/client/reset.png) no-repeat;
    border:none;
    cursor:pointer;
    float: left;
    height: 36px;
...
input.reset:hover
{
    background:url(<?= base_url(); ?>images/client/reset.png) 0 -36px no-repeat;
}
...
<body>
  ...
  <form action="<?php echo base_url()?>client_panel/checkuser" method="post">    
    <p>
      ...
      <input type="text" name='username' value="<?php if(isset($_POST['username'])){echo @$_POST['username'];} ?>"  class="rem_class <?php if(form_error('username')){echo 'error';} //echo set_value('username');  ?>"  />
    </p>
    <p>
      <label>Password&nbsp;</label>
      <input type="password" name="password" value="<?php if(isset($_POST['password'])){echo @$_POST['password'];} ?>" class="rem_class <?php if(form_error('password')){echo 'error';} ?>" />
    </p>
    <div style="clear:both"></div>    
    <input type="submit" value="" name="submit" class="buttom" /><input type="reset" value="" name="reset" class="reset" />
  </form>
</div>
</body>
</html>