...
<link type="text/css" rel="stylesheet" href="<?php echo base_url()?>css/logovote/jquery.ratings.css" />
<style type="text/css">
.jquery-ratings-star {
width: 16px;
height: 16px;
...
<script src="<?php echo base_url()?>js/logovote/example.js"></script>
...
<ul>
<?php
$dats = $this->db->query("select DISTINCT logo_name from logo_samples_to_client where logo_web_id=".$_SESSION['logoWebId']." GROUP BY logo_name ORDER BY logo_web_id")->result();
$my_Datas = $this->db->query("select * from tbl_logo_samples where logo_web_id=".$_SESSION['logoWebId']." ORDER BY id ASC")->result();
$dats1 = $this->db->query("select * from feedback_post")->result();
$count_also = count($dats);
$count_all = count($dats1);
$i=1;
...
foreach($dats as $qry)
{
$average_score = $this->db->query("SELECT sample_id, count(ratings),ROUND(AVG(ratings),2) FROM feedback_post where sample_id=".$i." and logo_web_id=".$_SESSION['logoWebId']."")->result_array();
$favourite_vote = $this->db->query("SELECT sample_id, count(winning_vote) FROM feedback_post where winning_vote!=0 and sample_id=".$i." and logo_web_id=".$_SESSION['logoWebId']."")->result_array();
$votes_count = $this->db->query("SELECT sample_id, count(sample_id) FROM feedback_post where sample_id=".$i." and logo_web_id=".$_SESSION['logoWebId']."")->result_array(); 
if($average_score[0]['count(ratings)']>0)
{
?>
<!-----live site = http://50dollarlogo.com/   localhost = http://localhost/50dl/------>
<?php /*foreach($dats as $qry)
{*/
?>
<li id="logo_select<?php echo $i?>"><img src="http://50dollarlogo.com/admin_panel/sample/<?php echo $qry->logo_name;?>" title="<?php echo $qry->logo_name;?>" height="120px" width="120px" class="select-this" id="logo<?php echo $i?>" />
<h3>Logo Sample No. <?php echo $i?></h3>
<?php //} ?>
<?php 
...
{*/
?>
<li id="logo_select<?php echo $i?>"><img src="http://50dollarlogo.com/admin_panel/sample/<?php echo $qry->sample_name;?>" title="<?php echo $qry->sample_name;?>" height="120px" width="120px" class="select-this" id="logo<?php echo $i?>" />
<h3>Logo Sample No. <?php echo $i?></h3>
<?php //} ?>
<?php 
foreach($average_score as $qrys){?>
...
<img src="<?= base_url(); ?>images/social_icons/avatar.gif" alt="" title="" />
</div>
