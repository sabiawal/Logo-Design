...
function calculatePrice() {
	...
	if(document.getElementById('taxamount'))
	{
		...
		totalPrice = parseFloat(totalPrice) + parseFloat(taxamount);				
	}	
	
	$('#LblYouPay').val(totalPrice);
	$('#LblYouPayTxt').text(totalPrice);
	$('#net_total').val(totalPrice);
  ...
  <div class="main-container">
   ...
   <?php 
	...
	?>
						...
						<td class="basketdesc" align="right" valign="top" ></td>                        
                        <td class="basketdesc" align="right" valign="top" colspan="2"  style="padding-right:10px;">'.CURRENCY.'<span id="taxamount">'.$tax_in_total.'</span></td>
                        </tr>';
                            ...
                            <!-- Calculation of promotional including tax -->
                              ...
                              <tr align="right">
                                <td width="175" rowspan="2" align="left" style="padding-left: 10px;"><span id="LblYouPayText" style="font-weight: bold;" class="totalpricetxt">Total </span></td>
                                <td width="14" rowspan="2" align="right">&nbsp;</td>
                                <td colspan="2" align="right" style="padding-right:10px;vertical-align:bottom;text-align:right"><input name="LblCurrencySign3" id="LblCurrencySign3" style="text-align: right; width: 15px;" class="pricetxt" readonly="readonly" value="<?php echo CURRENCY; ?>" type="text"/><input name="LblYouPay" id="LblYouPay" class="pricetxt" readonly="readonly" style="width: 45px; text-align: left;" value="<?php
                                @$_SESSION['subtotal'] = number_format(($total + @$_SESSION["price"] + @$_SESSION["option"] + @$_SESSION['web'] + @$_SESSION['brouchure']), 2, '.', '');
                                 echo number_format(@$_SESSION['subtotal'] + @$tax_in_total,2,".",""); ?>" type="hidden" /><span id="LblYouPayTxt" class="pricetxt"><?php
                                @$_SESSION['subtotal'] = number_format(($total + @$_SESSION["price"] + @$_SESSION["option"] + @$_SESSION['web'] + @$_SESSION['brouchure']), 2, '.', '');
                                 echo number_format(@$_SESSION['subtotal'] + @$tax_in_total,2,".",""); ?></span></td>
                              </tr>
              ...
              <tr id="basketbg" style="display: none;"><p>
                <td colspan="3" ><img src="<?php echo base_url()?>images/basket-image/basket2.gif" alt="basket-bottom1" width="365" height="17"/></p></td>
              </tr>
            </tbody>
          </table>
          <div id="pc_box">
        ...
        <div id="promotional">
        <label for="txtPc"> Enter Promotional code: </label><input type="text" name="txtPc" id="txtPc" /> <br />
        <p><span id="err" style="color:#C00;"></span>
        <input type="button" onclick="validatePromotional(document.getElementById('txtPc').value, '<?= @$_SESSION['insert']; ?>',document.getElementById('sub_total').value);" value="Update price" class="bluebutton1" id="btnUpdatePric" /></p>
      ...
      <div class="info-text-bottom">
        ...
        <div align="center">
          ...
          <p>We can offer this outstanding service as we produce high quality designs and achieve an industry<br />
            <span class="text-underline"> <strong>high 97% customer satisfaction rate.</strong></span> <br />
            Our clients are happy and only 3% ask for a refund. </p>
          <p>We will start working on your designs as soon as payment is received. We will email you the designs within 1-3 <br />
            days (24 hrs if ordered). </p>
          <p><strong>Please do not delay payment.</strong> This will delay you receiving the logo you require and slow your business<br />
            growth. Every successful business has a good logo.</p>
          <p>Act now and we guarantee 100% satisfaction or your money back!</p>
          <form action='<?php if(COUNTRY == "UK") {echo site_url('logodesign/paybankmid');} else{echo site_url('logodesign/jetpaymid'); }?>' method='post' name='frmOrder' enctype='multipart/form-data'>
            ...
            <input type="hidden" name="sub_total" id="sub_total" value="<?= @$_SESSION['subtotal']; ?>" />
            <input type="hidden" name="net_total" id="net_total" value="<?= @$_SESSION['subtotal'] + @$tax_in_total; ?> " />
