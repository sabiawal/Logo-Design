...
#promotional {
    background: url("<?php echo base_url()?>images/promotional-bg.png") no-repeat scroll -105px -80px #FFF345;
...
$(document).ready(function(){
	base_url = '<?=base_url()?>';  
	loadpage=base_url+'web-site-design/webdesigner-limited-sale/testing';
	//alert(loadpage);
	$("#aa").load(loadpage);
	...
	$('#updatePrice').live("click",function(){
		...
		$.ajax({			  
		  ...
		  success: function(data) {
			if(data=='wrong'){
				$('#wrong').text('this is invalid promotional code');	
			}
			else{
				$("#aa").load(loadpage);
			}
