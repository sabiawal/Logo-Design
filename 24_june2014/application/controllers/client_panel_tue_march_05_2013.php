...
class Client_panel extends CI_Controller {
	...
	public function index(){
		if($this->uri->segment(2)=='home.php');{
    ...
    //function to check valid user
		...
		public function home(){
			if($this->uri->segment(2)=='index.php');{
			$this->db->delete('logo_samples_to_client', array('logo_web_id' => $_SESSION['logoWebId']));
    ...
    public function logosample(){
		//if($this->uri->segment(2)=='logosample'){
			//$this->db->delete('logo_samples_to_client', array('logo_web_id' => $_SESSION['logoWebId'])); 
			...
			$condition = "(file_one != '' or file_two != '' or file_three != '' or file_four != '')";
	...
	public function businesscard(){
		if($this->uri->segment(2)=='businesscard.php');{
		$this->db->delete('logo_samples_to_client', array('logo_web_id' => $_SESSION['logoWebId']));
	...
	public function letterhead(){
		if($this->uri->segment(2)=='letterhead.php');{
		$this->db->delete('logo_samples_to_client', array('logo_web_id' => $_SESSION['logoWebId']));
	...
	public function complement(){
		if($this->uri->segment(2)=='complement.php');{
		$this->db->delete('logo_samples_to_client', array('logo_web_id' => $_SESSION['logoWebId']));
    ...
    public function clientfinal(){
		if($this->uri->segment(2)=='clientfinal.php');{
		$this->db->delete('logo_samples_to_client', array('logo_web_id' => $_SESSION['logoWebId']));
	...
	public function send_to_friends(){
		/*if($this->uri->segment(2)=='send_to_friends.php');{
		$this->db->delete('logo_samples_to_client', array('logo_web_id' => $_SESSION['logoWebId']));*/
        ...
        $this->load->view('client/send_to_friends');
    } 
	//}
	...
	public function logo_send_mail()
	{
		...
		//echo "<pre>"; print_r($logo_names); die;
	...
	public function logo_voted()
	{
		...
		$session_data1 = $this->session->set_userdata('show','1');
		...
		if($rate!=0 && $winning_votes=='1'){
		for($i=0;$i<$counts;$i++){
		if($split_data[1] == $_POST['logo-name'][$i]){
		$winning_votes = '1';
		}else{
		$winning_votes = '0';	
		}
		$all_data = array('ratings'=>$rate[$i],'ratings_for_sample'=>'sample-'.$logo_name[$i],'sample_id'=>$_POST['logo-name'][$i],'winning_vote'=>$winning_votes,'logo_web_id'=>$_SESSION['logoWebId']);
		$this->db->insert('feedback_post',$all_data);
		}
		}
		elseif($winning_votes=='1'){
		for($i=0;$i<$counts;$i++){
		$all_data = array('ratings'=>0,'ratings_for_sample'=>'sample-'.$split_data[1],'sample_id'=>$split_data[1],'winning_vote'=>$winning_votes,'logo_web_id'=>$_SESSION['logoWebId']);
		$this->db->insert('feedback_post',$all_data);
		}
		}
		elseif($rate!='0'){
		for($i=0;$i<$counts;$i++){
		$all_data = array('ratings'=>$rate[$i],'ratings_for_sample'=>'sample-'.$logo_name[$i],'sample_id'=>$_POST['logo-name'][$i],'winning_vote'=>$winning_votes1,'logo_web_id'=>$_SESSION['logoWebId']);
		$this->db->insert('feedback_post',$all_data);
		}
		}
		$messages = 'Dear '.$_POST["sender-email"].'<br><br>
					A new score or feedback has been given by your contacts.<br><br>					
					Please click on this link below to see:<br><br>	
...
<br>Thank you for your support.<br><br>Please feel free to leave some personal feedback below if you wish.');*/
		if(isset($_POST['send-feedback_x']) && isset($_POST['send-feedback_y'])){
		$messages_array = array('posted_user'=>$_POST['customer-fullname'],'posted_date'=>$posted_date,'posted_time'=>$posted_time,'description'=>$send_feedback,'logo_web_id'=>$_SESSION['logoWebId']);
...
<br>Thank you for your support.<br><br>Please feel free to leave some personal feedback below if you wish.');	
		...
		?><script>
		location.replace("<?php base_url();?>logovote?logo_id=<?php echo $_POST['logo-id']?>&<?php echo http_build_query(array('array'=>$_POST['logo_names']));?>&sender_email=<?php echo $_POST['sender-email']?>&receiver_email=<?php echo $_POST['receiver_email1']?>");</script>
        <?php
	}
    ...
    {
		if($this->uri->segment(2)=='copyright.php');{
		$this->db->delete('logo_samples_to_client', array('logo_web_id' => $_SESSION['logoWebId']));
