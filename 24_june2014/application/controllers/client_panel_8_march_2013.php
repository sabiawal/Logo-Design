...
class Client_panel extends CI_Controller {
	...
	public function send_to_friends(){	
		...
		$data['samples'] = $this->clientmodel->getSample($_SESSION['logoWebId'],$condition);
		//echo "<pre>"; print_r($data['samples']); die;
		$data['id'] = $_SESSION['logoWebId'];
	...
	public function logo_voted()
	{
		...
		$all_data = array('ratings'=>$rate[$i],'ratings_for_sample'=>'sample-'.$logo_name[$i],'sample_id'=>$_POST['logo-name'][$i],'winning_vote'=>$winning_votes,'logo_web_id'=>$_POST['client_id']);
		$this->db->insert('feedback_post',$all_data);
		}
		}
		elseif($winning_votes=='1'){
		for($i=0;$i<$counts;$i++){
		$all_data = array('ratings'=>0,'ratings_for_sample'=>'sample-'.$split_data[1],'sample_id'=>$split_data[1],'winning_vote'=>$winning_votes,'logo_web_id'=>$_POST['client_id']);
		$this->db->insert('feedback_post',$all_data);
		}
		}
		elseif($rate!='0'){
		for($i=0;$i<$counts;$i++){
		$all_data = array('ratings'=>$rate[$i],'ratings_for_sample'=>'sample-'.$logo_name[$i],'sample_id'=>$_POST['logo-name'][$i],'winning_vote'=>$winning_votes1,'logo_web_id'=>$_POST['client_id']);
		$this->db->insert('feedback_post',$all_data);
		}
		}
