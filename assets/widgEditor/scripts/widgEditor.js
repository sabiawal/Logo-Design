...
widgEditor.prototype.writeDocument = function(documentContent)
{
	...
	var documentTemplate = '\
		<html>\
			...
			<body id="iframeBody" placeholder="ddddd">\
				INSERT:CONTENT:END\
			</body>\
		</html>\
	';
...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		switch (widgToolbarItems[i])
		{
			...
			case "unorderedlist":
				this.addButton(this.theList.id + "ButtonUnordered", "widgButtonUnordered", "Bullets", "insertunorderedlist");
				
				break;
				
			case "orderedlist":
				this.addButton(this.theList.id + "ButtonOrdered", "widgButtonOrdered", "Numbers", "insertorderedlist");
