...
class Client_panel extends CI_Controller {
    ...
    public function __construct()
    {
		...
		$this->load->model('clientmodel');
                $ignore_views = array('login','checkuser','logovote','signin');
    ...
    public function how_works(){
        $this->load->view('client/how_works');
    } 
    public function whyus(){
        $this->load->view('client/whyus');
    } 
    public function adaward(){
        $this->load->view('client/adaward');
	}	
    public function saward(){
        $this->load->view('client/saward');
	}
    public function rdaward(){
        $this->load->view('client/rdaward');
	}
	...
	public function logo_vote_feedback(){
		...
		$all_files[] = $sels->file_one;
		}
		if(!empty($sels->file_two)){
		$all_files[] = $sels->file_two;
		}
		if(!empty($sels->file_three)){
		$all_files[] = $sels->file_three; 
		}
		if(!empty($sels->file_four)){
		$all_files[] = $sels->file_four;
		}
		if(!empty($sels->file_five)){
		$all_files[] = $sels->file_five;
		}
		if(!empty($sels->file_six)){ 
		$all_files[] = $sels->file_six;
		}
		if(!empty($sels->file_seven)){
		$all_files[] = $sels->file_seven;
		}
		if(!empty($sels->file_eight)){ 
		$all_files[] = $sels->file_eight;
		}
		if(!empty($sels->file_nine)){
		$all_files[] = $sels->file_nine;
		}
		if(!empty($sels->file_ten)){
		$all_files[] = $sels->file_ten;
		}
		if(!empty($sels->file_eleven)){ 
		$all_files[] = $sels->file_eleven;
		}
		if(!empty($sels->file_twelve)){
		$all_files[] = $sels->file_twelve; 
		}
		if(!empty($sels->file_thirteen)){
		$all_files[] = $sels->file_thirteen;
		}
		if(!empty($sels->file_14)){
		$all_files[] = $sels->file_14; 
		}
		if(!empty($sels->file_15)){
		$all_files[] = $sels->file_15;
		}
		...
		$logos = array('logo_name'=>$result[$i],'logo_web_id'=>$_SESSION['logoWebId']);
	...
	{ 
		...
		for($i=0;$i<$sum;$i++)
		{
			...
			$list_receiver['_email_'] = $add_receiver[$i];
...
<br>Thank you for your support.<br><br>Please feel free to leave some personal feedback below if you wish.');	
    ...
    public function copy_paybankmid()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $this->email->from(ORDER_MAIL,TEAM_NAME);
        ...
        if(isset($_GET['order_type']) && $_GET['order_type'] =="copyright" ) // thankyou page for Copyright section
        {
            if($payment_status == '0')
            {
                if((isset($_GET['copyright_id']) && $_GET['copyright_id'] != "" ) && (isset($_GET['authentication_code']) && $_GET['authentication_code'] == $authentication_code))
                {
                    ...
                    $this->email->initialize($config);
                    // send email for client about payment
                    $message = 'Dear '.$name.', <br /><br /><br /> Thank You for Your Pyament. <br /><br /> We have received Your Pyament Successfully.<br /><br /> We&rsquo;ll confirm your Copyright Registration by Email Soon. <br /><br /><br /> Thank You for using '.SITE_NAME2.'.<br />  '.TEAM_NAME;
                    $this->email->from(ORDER_MAIL,TEAM_NAME);
	...
	public function questions(){
        ...
        */
		...
		date_default_timezone_set('GMT'); 
		$current=date('Y-m-d H:i:s');
		$data=array('costumer_improv_id'=>$_SESSION['logoWebId'],'rate'=>'0','designeer_id'=>$data->assign,'current_time_gmt'=>$current);
		$this->clientmodel->insert('customer_rate',$data);
		//echo $this->db->last_query();
		$this->clientmodel->update('logo_web',array('status'=>'4'),array('logo_web_id'=>$_SESSION['logoWebId']));	
