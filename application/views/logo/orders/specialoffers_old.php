    ...
    if (mq.matches) {
        jQuery(document).ready(function ($) {
            if (window.history && window.history.pushState) {
                window.history.pushState('forward', null, 'orders/specialoffers/');
                $(window).on('popstate', function () {
                    window.location = "<?php echo base_url() ?>orders/special/";
                    return false;
                });
                    ...
                    <div class="leftcol">
                        ...
                        <div class="white-shade-box">
                            ...
                            <p>You currently have ordered <?php
                                ...
                                ?> logo concepts. This will massively increase the creativity and options for you. All services are backed by our <?php echo GUARANTEE_DAYS ?>-Day, 100% Money Back guarantee.</p>
                            <div class="cus-input-shell-cover text-center clearfix">
                                <div class="select-txt">Select here</div>
                                <div class="custom-input-shell">
    ...
    $(document).ready(function () {
        ...
        $('.select-qty').on('change', function () {
            ...
            if (chk_status.hasClass('chk')) {
                $.post("<?php echo site_url('orders/save_extra_options') ?>", {extra_options: row_class, val: val, extra_qty: extra_qty, 'size': size}, function (data) {
                    var opt = eval('(' + data + ')');
                    if (opt.total_tshirt) {
                        ...
                        $(".calc-box.cart").find("." + row_class).html('<span>' + opt.extra_options + '</span><span class="pull-right"><?php echo CURRENCY; ?><span class="package_price">' + opt.extra_options_price + '</span></span>');
                    }
                    get_total();
