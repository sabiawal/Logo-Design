...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('.pormo_update').click(function () {
            //alert('promotional');
            //alert($('.pcode').val());
            $.post("<?php echo site_url('reseller_web_orders/promo') ?>", {pcode: $('.pcode').val()}, function (data) {
                //alert(data);
                $('.tax_box').remove();
                if (data) {
                    var d = eval('(' + data + ')');
...
// To calculate total on page load
    function get_total() {
        ...
        var total_before_tax = gross_total;
<?php if (isset($_SESSION['reseller_id'])) { ?>
            var reseller_dis_rate = "<?php echo RESELLER_DISCOUNT; ?>";
            var reseller_discount = (total_price * reseller_dis_rate).toFixed(2);
            total_before_tax = total_price - reseller_discount;
<?php } ?>
       ...
       // var net_price = parseFloat(total_before_tax).toFixed(2);
        ...
        if (reseller_discount) {
            ...
            $(".calc-box.total").find(".reseller_discount").html('<span>Reseller Discount (10%)</span><span class="pull-right"><?php echo CURRENCY; ?><span >' + reseller_discount + '</span></span>');
        }
        /* if (purchase_dis > 0)
         {
        ...
        if (reseller_discount) {
            ...
            $(".calc-box.total").find(".gross_total").html('<span>Gross Total</span><span class="pull-right"><?php echo CURRENCY; ?><span>' + parseFloat(total_price).toFixed(2) + '</span></span>');
        }
    ...
    <div id="yellow-box-float" class="yellow-box">
        ...
        <div class="stack clearfix">
            <div class="calc-box  cart">                
                ...
                <?php if (isset($_SESSION['logo_package'])): ?>
                    <div class="inner-border-box row logo_package" > <span class="col-sm-8" ><?php echo $_SESSION['logo_package']; ?></span> <span class="col-sm-2">
                            <a href='javascript:void(0)' class='remove_package'  id="logo_package___<?php echo $_SESSION['logo_package'] ?>___logo_price___<?php echo $_SESSION['logo_price'] ?>" >
                                <img src='<?php echo base_url() . 'assets/images/remove.gif'; ?>' />
                            </a>
                        </span> 
                        <span class="col-sm-2 pull-right"><?php echo CURRENCY; ?>
                       <span class="package_price"><?php echo $_SESSION['logo_price']; ?></span></span> </div>
                <?php endif; ?>                
            </div>
            <div class="calc-box total">
                <?php if ($this->webdesignmodel->get_tax_rate(SITE_ID) > 0): ?>
                    <div class="inner-border-box tax_box"> <span>Tax <?php echo ($this->webdesignmodel->get_tax_rate(SITE_ID) * 100); ?>%</span> <span class="pull-right"><?php echo CURRENCY; ?><span class="tax_amount">0</span></span> </div>
                <?php endif; ?>
        ...
        <div class="calc-box re_calc-box clearfix">
            <?php if (!empty($_SESSION['deletedServices'])): ?>
                ...
                if ($services_value[0] == "logo_package") {
                    ?>
