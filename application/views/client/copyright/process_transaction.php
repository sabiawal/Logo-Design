		...
		else
		{
			...
			{			 
				header("Location: " . $GatewaySettings['PaymentApprovedPage'] . "?TransactionID=" . rawurlencode($retTransID) . "&ApprovalCode=" . rawurlencode($retApproval)
						. "&price=" . rawurlencode($newAmount)."&ip=".$ip."&copyright_id=".@$_SESSION['copyright_id'].'&authentication_code='.$_SESSION['authentication_code'].'&payment_method= Jetpay&order_type=copyright');
			}
			else
			{
