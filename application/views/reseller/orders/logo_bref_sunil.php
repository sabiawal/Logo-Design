<?php
$this->load->view('includes/reseller/header-common-order_sunil');
    ...
    <div class="container">
        <div class="row">
            <div class="col-xs-12 col-md-9 ">
                ...
                <form enctype="multipart/form-data" action="reseller_orders/special?uk=special" method="post" name="order_detail" id="order_detail">
                                        ...
                                        <script type="text/javascript">
                                            ...
                                            $(function () {
                                                ...
                                                function uploadFiles(event)
                                                {
                                                    ...
                                                    $('.loads').show();
                                                    $.ajax({
                                                        url: 'reseller_orders/uploadimagesfile',
                                                        type: 'POST',
                                                        data: data,
                                                        cache: false,
...
<?php } ?>
                        ...
                        <div class="normal-desc-box1 text-right1 ">
                            <div style="float:left;">
                                <a href="<?php echo base_url() ?>reseller_orders/index/<?php echo $_SESSION['p_index'] ?>" style="color:#666666;font-size:18px;"><i class="fa fa-chevron-left" aria-hidden="true"></i><span style="padding-left:17px;">Back </span></a></div>
                                  <!-- <a class="green-btn order-back-btn" href="<?php echo base_url() ?>reseller_orders/index/4">
...
<script>
    $(document).ready(function ()
    {
        $('textarea, input').on('input propertychange change', function ()
        {
            ...
            $.post("<?php echo base_url() ?>reseller_orders/autosave.php", name + '=' + value);
        });
        $('select').change(function ()
        {
            var name = this.name;
            var value = this.value;
            $.post("<?php echo base_url() ?>reseller_orders/autosave.php", name + '=' + value, function () {


                $.post("<?php echo base_url() ?>reseller_orders/update_floated/", {'name': name, 'value': value}, function (msg) {
                    if (name == 'web_package') {
...
<script type="text/javascript">
    $(function () {
        ...
        $('.fileList').delegate('.thremove', 'click', function () {
            ...
            $.post('<?php echo base_url() ?>reseller_orders/remove_attach', {'file_name': file_name, 'number': number, 'next_number': next_number}, function (msg) {
        ...
        $('#attachment').uploadify({
            ...
            onUploadSuccess: function (file, data, response) {
                ...
                /*if(prev_file != ''){
                 $.post('reseller_orders/prev_file/',{'prev_file':prev_file},function(msg){
                 //alert(msg);
                 });
                 }*/
                //alert(data);
                $.post('<?php echo base_url() ?>reseller_orders/prev_file/', {'fileList': data}, function (ms) {
                    $('#fileList').val(ms);
                });
                //	return false;
