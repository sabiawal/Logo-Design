<?php
class New44model extends CI_Model 
{
    ...
    function get_more_services($cat_id=NULL,$pack_id=NULL){
        $this->db->select(array('new_sub_packages.id as sub_package_id,new_sub_packages.id as sub_package_id,new_packages.id as package_id,categories','category_id','package_name','package_description2','package_type','usd','usd_reg','gbp','gbp_reg'));           
    ...
    function getmoreservices($sub_package_id=NULL){
        $this->db->select(array('new_sub_packages.id as sub_package_id,new_sub_packages.id as sub_package_id,new_packages.id as package_id,categories','category_id','package_name','package_description2','package_type','usd','usd_reg','gbp','gbp_reg'));           
        $this->db->from('new_sub_packages');            
        ...
        if($sub_package_id):
            //$where = "id <'45'";
            $where['new_sub_packages.id'] = $sub_package_id;
            //$where = array('category_id'=>$cat_id);            
            $this->db->where($where);
        endif;
        $pack_id= $_SESSION['new_package_id'];
            ...
            if (($query->num_rows()) > 0) {
                ...
                if ($row->payment == "1") {
                    ...
                    if (COUNTRY == 'USA') {
                        ...
                        $this->email->to('paymentnotification@logodesignguarantee.com');
                        $this->email->send();
                        $this->email->clear();
                    }
