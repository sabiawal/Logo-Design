...
class Logodesignmodel extends CI_Model {
    ...
    function save_logo_order_old() {
        ...
        if (isset($_SESSION['web'])) {
            $web = $_SESSION['web'];
        } elseif(isset($_SESSION['web_price'])) {
    ...
    function save_logo_order(){
       ...
       if (isset($_SESSION['logo_web_id']) && isset($_SESSION['reseller_id'])) {
        ...
        if ($_SESSION['fname'] == '0' || $_SESSION['company_name'] == '0')
            ...
            $query = $this->db->query("select * from order_extra where logo_web_id = ".$_SESSION['logo_web_id']);
            $extra_order = $query->result();
            if (isset($_SESSION['insert_extra']) && $extra_order) {
                $this->db->update('order_extra', $data1, array("extra_id" => $_SESSION['insert_extra']));
            }
            elseif (!isset($_SESSION['insert_extra']) && $extra_order) {
                $this->db->update('order_extra', $data1, array("logo_web_id" => $_SESSION['logo_web_id']));
            }else {
                $this->db->insert('order_extra', $data1);
                $_SESSION['insert_extra'] = $this->db->insert_id();
            }
