...
class Client_panel extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $ignore_views = array('index', 'checkuser', 'logovote', 'logo_voted');
    ...
    public function index() {
        if ($this->uri->segment(2) != 'signin') {
            if (isset($_SESSION['c_id'])) {
                ...
                //$this->load->view('client/logosample');
            } else {
...
//	}
    ...
    public function logo_voted() {
        ...
        $winning_vote = isset($_POST['winning-vote']) ? $_POST['winning-vote'] : "";
...
<br>Thank you for your support.<br><br>Please feel free to leave some personal feedback below if you wish.');
    ...
    public function signin($clientinfoid) {
        //die('here');
        @session_start();
    ...
    public function price_quotaion_request() {
    ...
    public function price_quotaion_sendmail() {
        if (isset($_POST['subject'])) {
            ...
            //$this->email->from('bks_prajapati@hotmail.com');
            $this->email->from($_SESSION['c_email']);
            $this->email->message($_POST['textarea']);
            for ($i = 0; $i < $no_files; $i++) {
                rename($_FILES['image']['tmp_name'][$i], $_FILES['image']['name'][$i]);
                $this->email->attach($_FILES['image']['name'][$i]);
            }
            $this->email->to('questions@logodesignguarantee.com');
    ...
    public function report_problem() {
        ...
        //echo '<pre>'; print_r($data['samples']);
        $data['id'] = $_SESSION['logoWebId'];
        $this->load->view('client/report_problem', $data);
    }

    public function report_problem_sendmail() {
        if (isset($_POST['subject'])) {
            ...
            $this->email->to('support@logodesignguarantee.com');
            //$this->email->to('bikash.designoffshore@gmail.com');
    ...
    public function finalStationary($final_id) {
...
//				$this->email->to('programmersuman@hotmail.com');// change it to yours
    ...
    public function process_transaction_1() {
        ...
        if (($query->num_rows()) > 0) {
            ...
            $data = array(
                ...
                'business_detail' => $row->business_detail,
                'package' => 'Custom Package',
                'price' => $_SESSION['subtotal'] - ($_SESSION['subtotal'] * 0.15),
                'site_id' => $row->site_id,
                'des_status' => $row->des_status,
                'payment' => '1',
                'submit_date' => $submit_date);
            ...
            $data_1 = array('logo_24hr' => $_SESSION['extra_package'],
                ...
                'order_cotton' => $bag,
                'order_tshirt' => $tshirt,
                'price_order_business' => $p_business_card,
                'price_order_letter_print' => $p_letterheads,
                'price_order_envelop_print' => $p_envelopes,
                'price_order_complement_print' => $p_compliment_slips,
                'price_order_mouse' => $qty_mouse_mat * $p_mouse_mat,
    ...
    public function session_unset_cart() {
        ...
        // echo '<pre>'; print_r($_SESSION); die;
        $amount_pay = ($_SESSION['subtotal'] - ($_SESSION['subtotal'] * 0.15));
