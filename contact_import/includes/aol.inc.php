<?php
$cookie_file_path = $tempdirpath.md5($_POST['username']).'.aol';
$ssl_ver=0;
		...
		{	
           ...
           // curl_setopt($ch,CURLOPT_COOKIEFILE,$cookie_file_path);
			...
			//form submitted
		  	...
		  	$path=str_replace("Lite/Today.aspx","",$location);
			$path=str_replace("Today.aspx","",$path); ////     -> /35861-111/aol-6/en-us/Lite/
			$path=str_replace("Suite.aspx","",$path); ///      ->/36032-111/aol-6/en-us/Suite.aspx -> /36032-111/aol-6/en-us/
			/// -> path= /36029-112/aol-6/en-us/
				
			$url="http://mail.aol.com".$path."Suite.aspx"; 
			...
			$url="http://mail.aol.com".$path."Lite/ContactList.aspx?folder=Inbox&showUserFolders=False";
			curl_setopt($ch, CURLOPT_URL,$url);
			curl_setopt($ch, CURLOPT_POST, 0);
			curl_setopt($ch,CURLOPT_HEADER,1);
			$return = curl_exec( $ch );
		    //we are in: http://mail.aol.com/36029-112/aol-6/en-us/Lite/ContactList.aspx?folder=Inbox&showUserFolders=False
			
			///go print contacts
			preg_match("|<input type=hidden name=\'user\' value=\'([^\']*)\' />|",$return,$getuserid);
			
			//http://mail.aol.com/36029-112/aol-6/en-us//AB/addresslist-print.aspx?command=all&undefined&sort=LastFirstEmailScreenNamePhone&sortDir=Ascending&nameFormat=LastFirstNick&user=jG-_dH2aF6
			$url="http://mail.aol.com".$path."AB/addresslist-print.aspx?command=all&undefined&sort=LastFirstEmailScreenNamePhone&sortDir=Ascending&nameFormat=LastFirstNick&user=".urlencode(@$getuserid[1]);
			$url="http://mail.aol.com".$path."Lite/addresslist-print.aspx?command=all&sort=FirstLastNick&sortDir=Ascending&nameFormat=FirstLastNick&user=".urlencode(@$getuserid[1]);
